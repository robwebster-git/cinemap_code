...
def Filter():
    ...
    if dist_filter == 'in':
        cont = cont
    filter = "A.NAME "+str(cont)+"IN (SELECT A.NAME FROM CINEMAS A, s0092179.HOODS_8307 P \
    WHERE P.NATURALCOM='"+str(dist_choice)+"' AND \
    SDO_GEOM.RELATE(A.GEOM, \
        'anyinteract', P.ORA_GEOMETRY, 0.5) = 'TRUE') "

    if dist_filter == 'adj':
        filter = "A.NAME IN (SELECT A.NAME FROM CINEMAS A WHERE SDO_GEOM.RELATE(A.GEOM, 'ANYINTERACT',(\
