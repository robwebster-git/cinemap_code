...
def foliumMap():
    ...
    folium.TileLayer('cartodbpositron', attr='Carto DB').add_to(map1)
    ...
    for row in c:
        ...
        name = row[1]
        titles.append(row[2])
        times.append(row[3:6])
        lon = row[6]
        lat = row[7]
        folium.Marker([lat, lon], popup=f"{name}\nFilm Times:\n{titles}, {times}", icon=folium.Icon(color='blue', icon='glyphicon-facetime-video')).add_to(cinema_layer)
